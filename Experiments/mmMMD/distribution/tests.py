...
class multiple_mMMD_test_statistic:
    """Calculates the chi-squared test statistic of mMMD with multiple kernels."""
    def __init__(self, kernels):
        """
        Args:
            kernels (list): A list of kernel functions. Each kernel must implement __call__(X, Y).
        """
        self.kernels = kernels

    def __call__(self, X, Y):
        """
        Calculates the mMMD test statistic with multiple kernels.
        
        Args:
            X, Y (np.ndarray): Input samples, each of shape (n, d).
        
        Returns:
            float: The chi-squared mMMD test statistic.
        """
        n = len(X)
        assert n == len(Y), "Sample sizes must match."

        r = len(self.kernels)
        
        # Store H_ell matrices and partial sums for each kernel
        S_list = []
        ...
        for k_idx, kernel in enumerate(self.kernels):
            ...
            H = K_XX - K_XY - K_YX + K_YY

            # Compute S_i^{(ell)} = (1/i) sum_{j=1}^{i-1} H_ell(i, j), for i = 2 to n
            S = np.zeros(n - 1)
            for i in range(1, n):  # i from 2 to n, index i-1 from 1 to n-1
                S[i - 1] = np.sum(H[i, :i]) / (i+1)
            S_list.append(S)

        # Build T_n = [T_{n,1}, ..., T_{n,r}]
        T = np.array([np.sum(S) for S in S_list])  # Each T_{n,l}

        # Build Sigma_{n,r}
        Sigma = np.zeros((r, r))
        for a in range(r):
            for b in range(r):
                Sigma[a, b] = np.sum(S_list[a] * S_list[b])

        # Regularize Sigma in case it's nearly singular
        Sigma += 1e-8 * np.eye(r)

        # Compute the test statistic: T^T Σ^{-1} T
        try:
            Sigma_inv = np.linalg.inv(Sigma)
        except np.linalg.LinAlgError:
            Sigma_inv = np.linalg.pinv(Sigma)  # Use pseudo-inverse if necessary

        test_stat = T.T @ Sigma_inv @ T
        return test_stat
